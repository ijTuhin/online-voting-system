...
if(isset($_POST['female_btn'])){        // for Female Database
...
if(isset($_POST['signup'])){
    ...
    /*
    if(){
        $gsuit_check = "SELECT * FROM usertable WHERE email = '$email'";
    } */
    ...
    $gs_check = "SELECT * FROM f_student WHERE gsuit = '$email'"; 
    $gs = mysqli_query($con, $gs_check);

//---------------***  GSUIT Verification
    if(mysqli_num_rows($gs) > 0){

    $u_check = "SELECT * FROM f_student WHERE s_id = '$s_id' AND gsuit = '$email'"; 
    $us = mysqli_query($con, $u_check);
    ...
    if(mysqli_num_rows($res) > 0){
        $errors['email'] = "You have registered already! Please login To continue.";
    }
    if(count($errors) === 0){
        ...
        $insert_data = "INSERT INTO usertable (name, s_id, email, mobile, address, password, photo, role, code, status, vstatus, vote, dept)
                        values('$name', '$s_id', '$email', '$mobile', '$address', '$encpass', '$image', '$role', '$code', '$status','0', '0', '$dept')";
        
        /*
        if($insert_data){
            echo '
                <script>
                    alert("Registration Successful!     We've sent a verification code to your email");
                    window.location = "../";
                </script>
            ';
        }
            ...
            echo '
                <script>
                    alert("Error!!       Failed while sending code!");
        ...
        */
        ...
        if($data_check){
            ...
            if(mail($email, $subject, $message, $sender)){
                ...
                header('location: home.php'); // Check the mail code later.
            } 
        }
        else{
...
else{    // Data not matched
    $errors['email'] = "Data does not match!! Provide Correct Information.";
}

    }

    else{       //GSUIT not identified
        $errors['email'] = "Please provide your Gsuit ID.";
    }
